...
- [5. packages/agent -- 에이전트 코어 런타임](#5-packagesagent--에이전트-코어-런타임)
  ...
  - [5.2 툴 실행 파이프라인](#52-툴-실행-파이프라인-병렬--순차)
  - [5.3 이벤트 시스템](#53-이벤트-시스템)
  - [5.4 훅 시스템](#54-훅-시스템-beforetoolcall--aftertoolcall)
  - [5.5 Steering & Follow-up 메시지](#55-steering--follow-up-메시지)
...
빌드 순서는 의존 그래프 기준으로 `tui -> ai -> agent -> coding-agent -> mom -> web-ui -> pods` 순이다.

### 1.2 pi-coding-agent 포지셔닝

출처: `packages/coding-agent/README.md`

Pi는 스스로를 "minimal terminal coding harness"로 정의한다. 핵심 사상은 "Pi를 워크플로우에 맞추지 말고, 워크플로우에 Pi를 적응시켜라"이다. Sub-agent, plan mode, permission popup, built-in to-do 같은 기능을 의도적으로 내장하지 않고, 대신 Extension, Skill, Prompt Template, Theme이라는 4가지 확장 축으로 사용자가 직접 구축하거나 제3자 패키지를 설치하도록 설계되었다.
...
이 철학의 핵심은 "코어를 최소화하되, 확장성을 극대화"하는 것이다. 기능 자체를 부정하는 것이 아니라, 기능이 코어에 포함되어야 할 필연성을 부정한다.
...
**Provider와 Api의 관계**: 하나의 Provider가 여러 Api를 가질 수 있다. `github-copilot`은 내부적으로 Anthropic/OpenAI 백엔드로 프록시하기 때문에 모델에 따라 `anthropic-messages`, `openai-responses`, `openai-completions` 세 가지 Api를 모두 사용한다. `groq`, `xai`, `cerebras` 같은 서드파티는 OpenAI 호환 엔드포인트를 제공하므로 `openai-completions` Api 하나에 얹힌다.
...
`convertToLlm()`은 `AgentContext` 전체를 변환하지 않는다. `AgentMessage[]` → `Message[]` 변환만 담당하며, `Context` 조립은 `agent-loop.ts`의 `streamAssistantResponse()`가 직접 수행한다:
...
const llmContext: Context = {
  systemPrompt: context.systemPrompt, // 그대로
  messages: llmMessages,              // 변환된 것
  tools: context.tools,               // AgentTool[] → Tool[] 업캐스팅 (변환 없음)
};

await streamSimple(config.model, llmContext, options); // pi-ai 호출
```
...
```
outer loop: follow-up 메시지가 있으면 계속
  │
  └─ inner loop: tool call 이 있거나 steering 메시지가 있으면 계속
       │
       ├─ (steering 메시지 있으면 context에 주입)
       ├─ streamAssistantResponse() → AssistantMessage → context에 누적
       ├─ tool call 있으면 executeToolCalls() → ToolResultMessage → context에 누적
       └─ turn_end emit

  inner loop 탈출 → getFollowUpMessages() 확인
  follow-up 없으면 outer loop 탈출 → agent_end emit
```
...
**인증 정보 해석 우선순위:**
1. CLI `--api-key` 플래그
2. `auth.json` 엔트리 (API 키 또는 OAuth 토큰)
3. 환경 변수
4. `models.json`의 커스텀 프로바이더 키
...
### 5.4 훅 시스템 (beforeToolCall / afterToolCall)
...
빌드 순서는 의존 그래프를 따른다: `tui -> ai -> agent -> coding-agent -> mom -> web-ui -> pods`
...
유저가 프롬프트를 입력한 시점부터 응답을 받기까지의 흐름. 화살표는 실제 제어 흐름, `Note`는 hook/event 발생 지점.

```mermaid
sequenceDiagram
    actor User
    participant CA as pi-coding-agent
    participant AC as pi-agent-core
    participant LLM as pi-ai

    User->>CA: 프롬프트 입력
    Note over CA: [hook] input — 인터셉트 / 변환 가능
    Note over CA: [hook] before_agent_start — 메시지 주입 / systemPrompt 수정 가능

    CA->>AC: agent.prompt(messages)
    Note over AC: [event] agent_start

    loop Turn Loop

        Note over AC: [event] turn_start
        Note over AC: [callback] transformContext — 메시지 pruning / 컨텍스트 주입
        Note over CA: [hook] context — LLM 호출 직전 메시지 수정 가능
        Note over CA: [hook] before_provider_request — payload 교체 가능

        AC->>LLM: streamSimple(model, context)
        Note over LLM: [callback] onPayload — 최하위 payload 검사 / 교체
        LLM-->>AC: AssistantMessageEventStream
        Note over AC: [event] message_start / message_update ×N / message_end

        alt LLM이 tool call 요청

            loop 각 tool call

                Note over AC: [event] tool_execution_start
                Note over CA: [hook] tool_call — 차단 가능 {block, reason}
                AC->>AC: tool.execute(params)
                Note over AC: [event] tool_execution_update ×N
                Note over CA: [hook] tool_result — 결과 수정 가능 {content, details, isError}
                Note over AC: [event] tool_execution_end

            end

            Note over AC: [callback] getSteeringMessages — 실행 중 메시지 주입

        else tool call 없음

            Note over AC: [callback] getFollowUpMessages — 추가 Turn 유발

        end

        Note over AC: [event] turn_end

    end

    Note over AC: [event] agent_end
    AC-->>CA: 완료
    CA->>User: 응답 표시
```
